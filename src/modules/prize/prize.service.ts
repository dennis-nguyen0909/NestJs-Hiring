...
export class PrizeService implements IPrizeService {
  ...
  async create(createPrizeDto: CreatePrizeDto): Promise<Prize> {
    const newPrize = await this.prizeModel.create(createPrizeDto);
    if (!newPrize) {
      throw new BadRequestException('Create prize failed');
    }
    const user = await this.userModel.findById(newPrize.user_id);
    if (!user) {
      throw new NotFoundException(`user #${newPrize.user_id} not found`);
    }
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (prize?.user_id === new Types.ObjectId(userId)) {
      throw new ForbiddenException('You are not allowed to delete this prize');
    }
  ...
  async findByOwner(ownerId: string): Promise<Prize[]> {
    return this.prizeModel.find({ owner: ownerId }).populate('owner').exec();
  }
}
